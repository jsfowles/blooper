...
function array_to_ui1(a) {
  return a[0];
}

function array_to_ui2be(a) {
  return a[1] + 256 * a[0];
}

function array_to_ui2le(a) {
  return a[0] + 256 * a[1];
}

function array_to_ui4be(a) {
  return 16777216 * a[0] + 65536 * a[1] + 256 * a[2] + a[3];
}

function array_to_ui4le(a) {
  return a[0] + 256 * a[1] + 65536 * a[2] + 16777216 * a[3];
}

function ui1_to_array(a, i, x) {
  a[i] = x;
}

function ui2be_to_array(a, i, x) {
  a[i + 0] = x / 256;
  a[i + 1] = x & 255;
}

function ui2le_to_array(a, i, x) {
  a[i + 1] = x / 256;
  a[i + 0] = x & 255;
}

function ui4be_to_array(a, i, x,
) {
  a[i + 0] = x / 16777216;
  a[i + 1] = (x / 65536) & 255;
  a[i + 2] = (x / 256) & 255;
...
const sendToMIDI = (data) => {
  try {
    _midi_output_port.send(_prop_to_midi(data));
  } catch (e) {
    console.error(e);
  }
};
...
function load_page() {
  ...
  _wave_dnload_data = [];
  _wave_dnload_length = _wave_dnload_offset = 0;
  _wave_upload_data = [];
  _wave_upload_count = _wave_upload_size = 0;
}
...
function _on_midi_message(event) {
  ...
  if (opcode == 8) {
    ...
    if (++_discover_session <= 16) {
      sendToMIDI([8, _discover_session, 0, 0, 0, 0]);
    } else {
      sendToMIDI([9, 0, 0, 0, 0, 0]);
    }
  }
  if (opcode == 9) {
    sendToMIDI([10, 0, 0, 0, 0, 0]);
  }
  ...
  if (opcode == 1) {
    ...
    sendToMIDI([8, _discover_session = 1, 0, 0, 0, 0]);
  ...
  // 0x0005/6/7 - Wavefile download
    ...
    if (opcode == 5 && _wave_dnload_data.length == 0) {
      ...
      if (_wave_dnload_length > 0) // Length
      {
        ...
        sendToMIDI([6, _wave_dnload_track, 0, 0, 0, 0]);
      } else {
    ...
    if (opcode == 7) {
      ...
      if (_wave_dnload_all) {
        ...
        if (_wave_dnload_track == 8) _wave_dnload_all = false;
          ...
          sendToMIDI([5, _wave_dnload_session, _wave_dnload_track, 0, 0, 0]);
        }
      }

      if (!_wave_dnload_all) {
        for (var ii = 1; ii <= 16; ++ii) $('dnload_' + ii + '_9').disabled = false;
        sendToMIDI([1, 0, 0, 0, 0, 0]);
      }
    }
  }
...
function _dnload_all_wavefiles(object) {
  ...
  sendToMIDI([5, _wave_dnload_session, 1, 0, 0, 0]);
}
...
function _prop_to_midi(property) {
  ...
  for (var ii = 0; ii < 6; ++ii) {
    midi_data.push((property[ii] >> 28) & 15);
    midi_data.push((property[ii] >> 24) & 15);
    midi_data.push((property[ii] >> 20) & 15);
    midi_data.push((property[ii] >> 16) & 15);
    midi_data.push((property[ii] >> 12) & 15);
    midi_data.push((property[ii] >> 8) & 15);
    midi_data.push((property[ii] >> 4) & 15);
    midi_data.push((property[ii] >> 0) & 15);
  }
  midi_data.push(0xF7);
  return midi_data;
}
...
function wave_to_samples(data) {
  ...
  var rate = 0;
  samples = [];
  var group_id = array_to_ui4le(data);
  data = data.slice(4); // Signature for 'RIFF'
  var total_size = array_to_ui4le(data);
  data = data.slice(4);
  var type_id = array_to_ui4le(data);
  data = data.slice(4); // Signature for 'WAVE'

  console.log(group_id, total_size, type_id);

  while (1) {
    if (data.length < 8) break;
    var blockid = array_to_ui4le(data);
    data = data.slice(4);
    var blocksz = array_to_ui4le(data);
    data = data.slice(4);
    ...
    if (blockid == 0x20746D66) // Signature for 'fmt'
    {
      ...
      format = array_to_ui2le(data);
      data = data.slice(2);
      channels = array_to_ui2le(data);
      data = data.slice(2);
      rate = array_to_ui4le(data);
      data = data.slice(4);
      thruput = array_to_ui4le(data);
      data = data.slice(4);
      align = array_to_ui2le(data);
      data = data.slice(2);
      width = array_to_ui2le(data);
      data = data.slice(2);
